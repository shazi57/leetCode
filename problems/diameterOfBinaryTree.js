function diameterOfBinaryTree(tree) {
  ...
  function maxDepth(root) {
    ...
    // Assign the left  of tree to LEFT;
    // this will be easier to call it instead of writing "maxDepth(root.left)" each time
  ...
  // since we don't know if the path will go through
  // the root or not we will have to get the max between(path that visits the root,
  // or the path that doesn't go through the root.)
  maxDepth(tree);
  return max;
}
